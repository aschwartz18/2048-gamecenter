...
function displayLine () {
    ...
    if (rodeoData['line'] == "orange") {
		...
		for (i = 0; i < numStations; i++) {
			...
			tstop.setMap(map);
			ptinfowindow = new google.maps.InfoWindow({content: findSchedule(station)});
			google.maps.event.addListener(tstop, 'click', function() {ptinfowindow.close(); ptinfowindow.open(map, tstop);});
...
/*
function makeMarker(loc, name) {
	var iconPic = "marker.png";
	var marker = new google.maps.Marker({position: loc, icon: iconPic});
	var pinfowindow = new infowindow
